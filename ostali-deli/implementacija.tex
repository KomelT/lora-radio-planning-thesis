...
Po pregledu sorodnih del sem se odločil, da aplikacije ne bom razvijal iz nič, temveč bom izvedel vejitev (angl.~\textit{fork}) obstoječega projekta \texttt{Meshtastic Site Planner}~\cite{meshtasticSitePlanner}, ki omogoča simulacijo širjenja radijskega signala. Projekt je licenciran pod licenco GPL~3~\cite{gpl3}, ki dovoljuje uporabo, spreminjanje in distribucijo programske opreme, pod pogojem, da so tudi izpeljana dela objavljena pod isto licenco. Tak licenčni model zagotavlja odprtost izvorne kode ter omogoča, da so nadaljnje izboljšave dostopne širši skupnosti, ne le posameznemu razvijalcu.

Namesto razvoja osnovne infrastrukture aplikacije sem se osredotočil na razširitve in nadgradnje, kot so integracija simulacijskega orodja SPLAT!, optimizacija prikaza rezultatov na interaktivnem zemljevidu ter dodajanje funkcionalnosti za iskanje optimalnih lokacij oddajnikov. S tem sem povečal učinkovitost razvoja in hkrati prispeval k obstoječemu odprtokodnemu ekosistemu.
...
\subsection{Izračun potrebnih \texttt{.sdf} ploščic za vidno linijo}
Ker izvorna aplikacija ni podpirala preverjanja vidne linije, sem implementiral funkcijo, ki izračuna, katere ploščice (angl.~\textit{tiles}) so potrebne za izvedbo simulacije.
Za preverjanje vidne linije (LOS) med oddajnikom $(\varphi_\mathrm{tx}, \lambda_\mathrm{tx})$ in sprejemnikom $(\varphi_\mathrm{rx}, \lambda_\mathrm{rx})$ potrebujemo zgolj tiste SRTM ploščice, ki jih seka zveznica med točkama. Namesto prenosa večjega nabora ploščic za širše območje se uporabi algoritmičen sprehod po mreži ploščic, ki v času $O(n)$ (kjer je $n$ število prečkanih ploščic) deterministično izdela seznam natančno tistih celic, ki jih daljica prečka. S tem se zmanjša količina podatkov za prenos in skrajša priprava simulacije.

\paragraph{Diskretizacija prostora.}
Koordinate obravnavamo v stopinjah in prostor razdelimo na celice velikosti $1^\circ \times 1^\circ$. 
Za parametrični opis daljice med končnima točkama vzamemo $(x(t), y(t)) = (x_0, y_0) + t\,(dx, dy)$, $t \in [0,1]$, kjer $x$ predstavlja geografsko dolžino in $y$ geografsko širino, $dx = x_1 - x_0$ in $dy = y_1 - y_0$.

\paragraph{DDA sprehod po mreži.}
Uporabimo varianto algoritma DDA (Digital Differential Analyzer) oz. mrežnega sledenja žarku:
\begin{enumerate}
  \item Začetno celico določimo z $\lfloor x_0 \rfloor, \lfloor y_0 \rfloor$, ciljno pa z $\lfloor x_1 \rfloor, \lfloor y_1 \rfloor$.
  \item Izračunamo korake $\,\mathrm{stepX} \in \{-1,0,1\}$ in $\,\mathrm{stepY} \in \{-1,0,1\}$ glede na predznak $dx$ in $dy$.
  \item Določimo \emph{tDelta} vrednosti, ki povedo, kolikšen porast parametra $t$ je potreben, da prečkamo eno celično mejo po $x$ oziroma $y$:
  \[
    t_{\Delta x} = 
    \begin{cases}
      |1/dx|, & dx \neq 0\\
    ...
    \end{cases}
  \]
  \item Inicializiramo \emph{tMax} vrednosti, tj. parametrični čas do prve vertikalne oziroma horizontalne meje celice od izhodišča:
  \[
    t_{\max x} = 
    \begin{cases}
      \dfrac{(\lfloor x_0 \rfloor + 1 - x_0)}{dx}, & dx > 0\\[6pt]
      \dfrac{(x_0 - \lfloor x_0 \rfloor)}{-dx}, & dx < 0\\[6pt]
      \infty, & dx = 0
    \end{cases}
    \quad
    t_{\max y} = 
    \begin{cases}
      \dfrac{(\lfloor y_0 \rfloor + 1 - y_0)}{dy}, & dy > 0\\[6pt]
      \dfrac{(y_0 - \lfloor y_0 \rfloor)}{-dy}, & dy < 0\\[6pt]
      \infty, & dy = 0
    \end{cases}
  \]
  \item Iterativno napredujemo v smeri manjše od $t_{\max x}$ ali $t_{\max y}$ (prečkamo naslednjo mejo celice po $x$ ali $y$), s čimer posodobimo $(\mathrm{current\_x}, \mathrm{current\_y})$ ter $t_{\max x} \leftarrow t_{\max x} + t_{\Delta x}$ ali $t_{\max y} \leftarrow t_{\max y} + t_{\Delta y}$. 
  Postopek ponavljamo, dokler ne dosežemo ciljne celice.
\end{enumerate}

\subsection{Izračun potrebnih ploščic za simulacijo radijske pokritosti}
Simulacija radijske pokritosti in s tem povezan izračun potrebnih ploščic sta bila že implementirana v izvorni aplikaciji.

Postopek določanja potrebnih ploščic za simulacijo radijske pokritosti je zasnovan podobno kot pri izračunu ploščic za vidno linijo v prejšnjem poglavju.
Namesto izračuna med dvema točkama pa se v tem primeru upošteva celoten okoliš oddajnika znotraj izbranega polmera $r$.

\paragraph{Določitev območja pokritosti.}
Najprej se glede na izbrani polmer in geografsko širino oddajnika izračuna kotni odmik $\Delta_{\deg}$:
\[
\Delta_{\deg} = \frac{r}{R_\oplus}\cdot \frac{180}{\pi},
\]
kjer je $R_\oplus = 6\,378\,137$~m približen polmer Zemlje.  
Na tej osnovi se določi omejitveni pravokotnik (angl.~\textit{bounding box}) z mejami:
\[
\varphi_{\min} = \varphi_0 - \Delta_{\deg}, \quad
\varphi_{\max} = \varphi_0 + \Delta_{\deg}, \quad
\lambda_{\min} = \lambda_0 - \frac{\Delta_{\deg}}{\cos(\varphi_0)}, \quad
\lambda_{\max} = \lambda_0 + \frac{\Delta_{\deg}}{\cos(\varphi_0)}.
\]
Omejitve so podane v stopinjah in določajo minimalni ter maksimalni razpon zemljepisne širine in dolžine, ki ju mora sistem obdelati.
...
\begin{itemize}
  ...
  \item \texttt{-olditm} – uporaba standardnega modela ITM (Longley–Rice).
\end{itemize}
