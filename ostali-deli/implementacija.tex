...
SPLAT! za določitev geografskih lokacij oddajnika in sprejemnika uporablja \textbf{.qth} datoteke (angl.~\emph{QTH files}), 
...
\begin{enumerate}
  ...
  \item višina antene nad tlemi (\emph{AGL – Above Ground Level}) v metrih ali čevljih.
\end{enumerate}
...
Geografska širina in dolžina se lahko zapišeta v decimalnem formatu (npr.~\texttt{46.0500}) ali v formatu stopinje–minute–sekunde (npr.~\texttt{46 3 0.0}).  
...
(angl.~\emph{Irregular Terrain Model Parameter file}), ki določa fizikalne in statistične parametre modela 
...
Če je zadnji parameter (ERP) izpuščen, SPLAT! izračuna zgolj \emph{path loss} (izgubo poti).  
Če je vključen, pa izvede tudi izračun jakosti električnega polja in kontur pokritosti (\emph{field strength}).
