...
\noindent\begin{tabular}{p{0.11\textwidth}|p{.39\textwidth}|p{.39\textwidth}}    % po potrebi razširi prvo kolono tabele na račun drugih dveh!
  ...
  {\bf CSS} & Cascading Style Sheets & Kaskadni slogovni listi \\
  {\bf DEM} & Digital Elevation Model & Digitalni model višin \\
  {\bf ERP} & Effective Radiated Power & Efektivna sevalna moč \\
  {\bf EPSG} & European Petroleum Survey Group (geodetic parameter dataset) & EPSG (nabor geodetskih parametrov) \\
  {\bf GDAL} & Geospatial Data Abstraction Library & Knjižnica za obdelavo geospacijskih podatkov \\
  {\bf GeoJSON} & Geographic JavaScript Object Notation & Geografski zapis JSON \\
  {\bf GeoTIFF} & Geographic Tagged Image File Format & Geografski format TIFF \\
  {\bf GIS} & Geographic Information System & Geografski informacijski sistem \\
  {\bf GPL} & GNU General Public License & GNU splošna javna licenca \\
  {\bf HTTP} & Hypertext Transfer Protocol & Protokol za prenos nadbesedila \\
  {\bf HTTPS} & Hypertext Transfer Protocol Secure & Varni protokol za prenos nadbesedila \\
  {\bf ISM} & Industrial, Scientific and Medical & Industrijski, znanstveni in medicinski (frekvenčno območje) \\
  {\bf ITM} & Irregular Terrain Model & Model neravnega terena (Longley--Rice) \\
  {\bf ITWOM} & Irregular Terrain with Obstructions Model & Model neravnega terena z ovirami \\
  {\bf IoT} & Internet of Things & Internet stvari \\
  {\bf JSON} & JavaScript Object Notation & Notacija JavaScript objektov \\
  {\bf KML} & Keyhole Markup Language & Označevalni jezik KML \\
  {\bf LOS} & Line of Sight & Vidna linija \\
  {\bf LoRa} & Long Range & Dolg doseg \\
  {\bf LPWAN} & Low Power Wide Area Network & Nizkoenergijsko širokoobmočno omrežje \\
  {\bf MQTT} & Message Queuing Telemetry Transport & Telemetrijski protokol MQTT \\
  {\bf NASA} & National Aeronautics and Space Administration & Nacionalna uprava za aeronavtiko in vesolje \\
  {\bf NED} & National Elevation Dataset & Nacionalna zbirka višin \\
  {\bf NLOS} & Non-Line of Sight & Brez vidne linije \\
  {\bf OGC} & Open Geospatial Consortium & Odprti geoinformacijski konzorcij \\
  {\bf PDF} & Portable Document Format & Prenosni dokumentni format \\
  {\bf QGIS} & Quantum GIS & QGIS (geografski informacijski sistem) \\
  {\bf RaPlaT} & Radio Propagation and Terrain Analysis Tool & Orodje za analizo širjenja radijskega signala in terena \\
  {\bf RF} & Radio Frequency & Radijska frekvenca \\
  {\bf RSSI} & Received Signal Strength Indicator & Kazalnik jakosti sprejetega signala \\
  {\bf SHP} & ESRI Shapefile & Shapefile (format ESRI) \\
  {\bf SNR} & Signal-to-Noise Ratio & Razmerje signal--šum \\
  {\bf SPLAT} & Signal Propagation, Loss, And Terrain analysis tool & Orodje za analizo širjenja signala, izgub in terena \\
  {\bf SRTM} & Shuttle Radar Topography Mission & Misija radarske topografije terena \\
  {\bf SRTM1} & Shuttle Radar Topography Mission (1 arc-second) & SRTM z ločljivostjo 1~ločne sekunde \\
  {\bf SRTM3} & Shuttle Radar Topography Mission (3 arc-seconds) & SRTM z ločljivostjo 3~ločnih sekund \\
  {\bf USGS} & United States Geological Survey & Geološki zavod ZDA \\
  {\bf WCS} & Web Coverage Service & Spletna storitev pokritosti \\
  {\bf WFS} & Web Feature Service & Spletna storitev vektorskih značilnic \\
  {\bf WGS84} & World Geodetic System 1984 & Svetovni geodetski sistem 1984 \\
  {\bf WMS} & Web Map Service & Spletna kartografska storitev \\
%  \dots & \dots & \dots \\
\end{tabular}
...
\include{ostali-deli/uvod}
\include{ostali-deli/sorodna-dela}
\include{ostali-deli/tehnologije-in-orodja}
\include{ostali-deli/arhitektura-resitve}
\include{ostali-deli/implementacija}
\include{ostali-deli/eksperimentalna-evalvacija}
\include{ostali-deli/diskusija}
\include{ostali-deli/zakljucek}
